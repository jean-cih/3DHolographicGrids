...
1. Изменение пространственного периода решетки:
- При увеличении длины волны записывающего излучения (λ) пространственный период решетки (d) увеличивается.
2. Изменение пространственных частот:
- Пространственные частоты уменьшаются с увеличением длины волны, так как они обратно пропорциональны периоду:
3. Влияние среды:
- В среде с большим показателем преломления (n = 1.57) длина волны уменьшается, что приводит к уменьшению периода решетки по сравнению с воздухом. Однако в таблицах видно, что период в среде немного больше, что может быть связано с учетом углов падения и Брэгга.
4. Изменение углов падения и Брэгга:
- Углы падения и Брэгга остаются постоянными для одной и той же геометрии (θS = -15°, θR = 0°), но их значения зависят от среды. В среде углы уменьшаются из-за преломления:
5. Особенности вдоль оси Z (dz, vz):
- Период вдоль оси Z (dz) значительно больше, чем в плоскости XY, и сильно зависит от длины волны и среды:
- В среде dz увеличивается ещё больше из-за уменьшения эффективной длины волны.
6. Интенсивность распределения:
- Распределение интенсивности зависит от длины волны и может показывать различную степень дифракционной эффективности. Например, для больших длин волн (λ = 1.5 мкм) решетка может иметь менее резкие границы из-за увеличенного периода.
...
При ΩS = -25° решетка имеет меньший период и более высокую пространственную частоту по сравнению с ΩS = -15°. Однако с увеличением λ сохраняется общая тенденция: рост периода и снижение частоты. 
...
Конфигурация с ΩS = -35° демонстрирует наиболее высокочастотные решетки среди рассмотренных случаев, что делает её перспективной для применений, требующих высокой разрешающей способности. Однако при этом уменьшается дифракционная эффективность для больших λ. Учет этих особенностей важен при проектировании оптических устройств, таких как спектральные фильтры или датчики.


#### **Вывод**
Анализ численных данных из таблиц подтверждает закономерности, наблюдаемые на рисунках распределения интенсивности. Ключевые выводы:
1. Угол Брэгга сохраняет постоянное значение при изменении длины волны, что свидетельствует о его зависимости исключительно от геометрии системы.
2. Основное влияние на параметры решетки оказывает пространственный период, величина которого:
   - определяется разностью углов падения сигнальной и референтной волн
   - уменьшается с увеличением этой разности
   - что приводит к пропорциональному росту пространственной частоты
3. Проведенные расчеты для среды с показателем преломления n₂ = 1.57 демонстрируют:
   - возможность формирования согласованных периодических структур
   - как в оптической среде, так и в воздушной среде
   - с сохранением основных закономерностей, но с корректировкой абсолютных значений параметров
Полученные результаты подтверждают теоретические предпосылки и демонстрируют устойчивость наблюдаемых закономерностей при изменении условий эксперимента.
...
В ходе компьютерного моделирования в среде NVIM (Python 3.12) были исследованы характеристики голографических решеток, формирующихся при интерференции 2-х световых пучков на фоточувствительном материале. Анализ полученных 3D-моделей и расчётных параметров для сред с разными показателями преломления позволил выявить следующие закономерности:
1. Ориентация решётки:
- Наклон интерференционной структуры определяется исключительно геометрией записи (разницей углов между сигнальным и референтным пучками)
- Данная характеристика остаётся неизменной при варьировании длины волны
2. Геометрические зависимости:
- Увеличение угла падения сигнального пучка приводит к возрастанию угла наклона решётки
- Угол Брэгга, будучи инвариантным к длине волны, изменяется только при модификации взаимной ориентации записывающих пучков
3. Пространственные параметры:
- Период решётки демонстрирует линейную зависимость от длины волны излучения
- Частота структур обратно пропорциональна длине волны
- При увеличении углового расхождения пучков наблюдается:
  • Рост пространственной частоты
  • Уменьшение периода решётки
Полученные результаты подтверждают теоретические предсказания и демонстрируют возможность точного управления параметрами решёток путём изменения условий записи. Особый интерес представляет сравнение характеристик структур, сформированных в воздушной среде и в материале с повышенным показателем преломления.
